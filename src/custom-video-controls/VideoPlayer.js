...
export default class VideoPlayer extends Component {
    ...
    constructor( props ) {
        ...
        let quickie_timecodes = [];
        if( this.props.quickie_timecodes ) {
            quickie_timecodes = this.props.quickie_timecodes;
        }
        this.state = {
            // Video
            ...
            error: false,
            duration: 0,
            seekOnStart: (this.props.lastWatchedPoint > 0),
            quickie_timecodes: this.props.quickie_timecodes,
            show_quickie_markers: false,
        };
        ...
        this.events = {
            ...
            onPlayerInit: this._onPlayerInit.bind( this),
        };
    ...
    //Force setting of paused
    componentWillReceiveProps(nextProps) {
        // if( nextProps.playerPaused != this.state.playerPaused ) {
    ...
    */
    ...
    calculateSeekbarQuickieMarkers() {
        let quickieSeekbarMarkers = [];
        for( let i=0;i<this.state.quickie_timecodes.length;i++){
          let markerWidthPosition = (this.state.quickie_timecodes[i].in / this.state.duration) * this.player.seekerWidth;
          if( this.state.quickie_timecodes[i].out > this.state.quickie_timecodes[i].in) {
            quickieSeekbarMarkers.push(markerWidthPosition);
          }
        }
        return quickieSeekbarMarkers;
    }
    ...
    _onLoad = ( e = {} ) => {
        ...
        //state.loading = false;
        if( this.state.quickie_timecodes.length > 0 ) {
            state.show_quickie_markers = true;
        }
        this.setState( state );
        ...
        if( this.state.seekOnStart ) {
          this.seekTo(this.props.lastWatchedPoint);
          this.state.seekOnStart = false;
        }
    ...
    _onProgress( e = {} ) {
      ...
      let state = this.state;
        ...
        if( this.props.onProgress ) {
            this.props.onProgress( {currentTime: currentTime} );
        }
    }

    _onPlayerInit( e = {} ) {
        const {drmID} = e.nativeEvent;

        if( this.props.onPlayerInit ) {
            this.props.onPlayerInit( {drmID: drmID});
        }
    }
    ...
    _onError( err ) {
      ...
      console.log('Error - ' + JSON.stringify(err, function(key, val) {
    ...
    */
    ...
    _togglePlayPause() {
        ...
        // this.setState( {paused: state.paused, playerPaused: state.paused} );
    }
    ...
    formatTime( time = 0 ) {
        ...
        const hoursMod = Math.floor(time % 3600);

        const minutes = time / 60;
        let minutesFloored = Math.floor(minutes);
        if( minutes >= 60 ) {
          minutesFloored = Math.floor(minutes % 60);
        }

        const seconds = time % 60;

        const formattedHours = _.padStart( hours.toFixed( 0 ), 2, 0 );
        const formattedMinutes = _.padStart( minutesFloored.toFixed( 0 ), 2, 0 );
        ...
        return(
            <Animated.View style={[
                ...
                <Image
                    ...
                    <View pointerEvents="auto" style={ {alignItems:'center', justifyContent:'flex-start'} }>
                      { this.renderBack() }
                    </View>
...
// Temporarily remove fullscreen button
// { this.renderFullscreen() }
    ...
    renderBottomControls() {
        return(
            ...
            <Image
                ...
                style={[ { height:null, width:null}, styles.controls.vignette
            ]}>
                <View style={[
                    { justifyContent: 'space-between' },
    ...
    renderQuickieMarkers(){
        let returnViewItems = [];
        let seekbarQuickieMarkers = this.calculateSeekbarQuickieMarkers();

        for( let i=0;i<seekbarQuickieMarkers.length;i++) {
            returnViewItems.push((
              <View key={i} style={{
                position: 'absolute',
                left: seekbarQuickieMarkers[i],
                top: -1,
                width: 4,
                height: 4,
    ...
    renderSeekbar() {
        return (
            <View style={ styles.seek.container }>
                ...
                >
                    <View>
                        { this.state.show_quickie_markers ? this.renderQuickieMarkers(): null }
                    </View>
                </View>
                <View style={[
                    styles.seek.fill,
     ...
     */
    renderPlayPause() {
        let source = this.props.paused === true ? require( './assets/img/ic_play.png' ) : require( './assets/img/ic_pause.png' );
        return this.renderControl(
            <Image source={ source } />,
            this.methods.togglePlayPause,
            [styles.controls.playPause,{justifyContent: 'center', alignItems:'center', height: 100}]
        );
    }
            ...
            >
                <View style={[ styles.player.container, this.props.style ]}>
                    <OSPlayer
                        ...
                        style={[ styles.player.video, this.props.style ]}

                        source={ this.props.source }

                        onPlayerInit={ this.events.onPlayerInit }
